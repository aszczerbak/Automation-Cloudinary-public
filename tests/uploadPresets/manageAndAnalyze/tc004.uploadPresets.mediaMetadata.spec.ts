...
/**
 * TC ID: TC004-UploadPresets-MediaMetadata
 * Title: Upload Presets - Media metadata
 *
 * Description:
 * Creates an Upload Preset, enables "Retrieve media metadata" in "Manage and Analyze",
 * saves, verifies details show "media metadata: true", then deletes the preset.
 *
 * Steps:
 * 1) Open Upload Presets
 * 2) Create preset
 * 3) Go to "Manage and Analyze"
 * 4) Enable media metadata
 * 5) Save
 * 6) Verify details show "media metadata: true"
 * 7) Delete preset (cleanup)
 */
test('Upload Presets: media metadata', async ({ page }) => {
  ...
  const presetName = `e2e_metadata_${randomString()}`;
  ...
  // Step 5: enable "Retrieve media metadata"
  await presetsPage.ensureSwitchOn(UploadPresets.retrieveMediaMetadataSwitch(page));
  ...
  try {
    ...
    // Step 8: verify details show media metadata enabled
    await expect(UploadPresets.detailsText(page, /media metadata:\s*true/i)).toBeVisible();
  } finally {
