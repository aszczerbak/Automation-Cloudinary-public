...
/**
 * TC ID: TC008-UploadPresets-RetrieveFacesCoordinates
 * Title: Upload Presets - Retrieve face coordinates
 *
 * Description:
 * Creates an Upload Preset, enables "Retrieve faces coordinates" in "Manage and Analyze",
 * saves, verifies details show "faces: true", then deletes the preset.
 *
 * Steps:
 * 1) Open Upload Presets
 * 2) Create preset
 * 3) Go to "Manage and Analyze"
 * 4) Enable "Retrieve faces coordinates"
 * 5) Save
 * 6) Verify details show "faces: true"
 * 7) Delete preset (cleanup)
 */
test('Upload Presets: retrieve face coordinates', async ({ page }) => {
  ...
  const presetName = `e2e_faces_${randomString()}`;
  ...
  // Step 5: enable "Retrieve faces coordinates"
  await presetsPage.ensureSwitchOn(UploadPresets.retrieveFacesCoordinatesSwitch(page));
  ...
  try {
    ...
    // Step 8: verify faces coordinates enabled
    await expect(UploadPresets.detailsText(page, /faces:\s*true/i)).toBeVisible();
  } finally {
