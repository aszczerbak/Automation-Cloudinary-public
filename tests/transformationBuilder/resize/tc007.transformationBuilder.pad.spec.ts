/**
 * TC ID: TC007-TransformationBuilder-Resize-Pad
 * Title: Transformation Builder - Pad Resize Test
 *
 * Description:
 * Tests the Pad (c_pad) transformation mode which resizes the image to fit within
 * the specified dimensions and adds padding to reach the exact dimensions.
 *
 * Steps:
 * 1) Navigate to Transformation Builder
 * 2) Apply Pad transformation with 500x500 dimensions
 * 3) Save transformation with unique name
 * 4) Open saved transformations list and locate the saved transformation
 * 5) Open transformation card and verify parameters contain c_pad,h_500,w_500
...
test('TB: pad 500x500 -> save as -> verify params + image size -> delete', async ({ page }) => {
  ...
  // 1) Apply Pad 500x500
  await tb.applyResize('Pad', width, height);
  ...
  await tb.expectParamsContains(`c_pad,h_${String(height)},w_${String(width)}/`);
