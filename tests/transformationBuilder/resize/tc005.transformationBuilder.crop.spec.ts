/**
 * TC ID: TC005-TransformationBuilder-Resize-Crop
 * Title: Transformation Builder - Crop Resize Test
 *
 * Description:
 * Tests the Crop (c_crop) transformation mode which extracts a region from
 * the original image at the exact specified dimensions.
 *
 * Steps:
 * 1) Navigate to Transformation Builder
 * 2) Apply Crop transformation with 500x500 dimensions
 * 3) Save transformation with unique name
 * 4) Open saved transformations list and locate the saved transformation
 * 5) Open transformation card and verify parameters contain c_crop,h_500,w_500
...
test('TB: crop 500x500 -> save as -> verify params + image size -> delete', async ({ page }) => {
  ...
  // 1) Apply Crop 500x500
  await tb.applyResize('Crop', width, height);
  ...
  await tb.expectParamsContains(`c_crop,h_${String(height)},w_${String(width)}/`);
